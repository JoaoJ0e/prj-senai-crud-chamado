...
public class ConsultaClienteController {
    ...
    private void editarCliente(ClienteDto clienteDto) throws IOException {
        boolean editarTécnico = StartViewController.mostrarTelaEditar("cadastro-cliente.fxml", "Editar Técnico", clienteDto, new CadastroClienteController());
        if(editarTécnico)recarregarTabela();
    }

    public void recarregarTabela() {
        clienteList.clear(); // Limpa a lista existente
        clienteList.addAll(clienteService.listarTodos()); // Carrega novamente a lista de técnicos
